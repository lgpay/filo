...
const state = {
  config: null,
  dirs: [],          // flat menu array from /api/dirs
...
// Load the file tree + current directory. Called after a successful login.
async function bootFilo() {
  const menu = await api('/api/dirs');
  state.dirs = menu;
  state.expanded.add('');   // Home 根节点默认展开，露出根下的子目录与文件
  renderMenu();
  ensureFiles('');          // 预加载根目录（Home）下的文件叶子
  buildLayoutMenu();
  buildSortMenu();
...
async function loadDir(rel) {
  ...
  try {
    const dir = await api('/api/dir', { path: rel });
    if (dir && dir.error) {
...
function filesArray(dir) {
  return Object.values(dir.files || {});
}
...
async function ensureFiles(path) {
  ...
  try {
    const dir = await api('/api/dir', { path });
    if (dir && !dir.error) {
      ...
      state.fileCache.set(path, Object.values(dir.files || {}).filter((f) => !f.is_dir));
