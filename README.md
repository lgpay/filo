...
一个用 **Node.js + 原生 JavaScript** 构建的轻量级 **文件管理系统**：目录树浏览、缩略图网格、预览灯箱，并内置上传 / 新建 / 移动 / 复制 / 重命名 / 删除 / ZIP 下载等完整的文件管理操作。

它最初受 Files Gallery 启发，但定位已从「图片相册」演进为通用的「文件管理」——既能管理图片（自动生成缩略图），也能管理任意类型的文件（以类型图标呈现）。
...
![登录门禁](screenshots/login.png)
![登录后的文件列表](screenshots/gallery.png)
![设为公开的目录](screenshots/public.png)
